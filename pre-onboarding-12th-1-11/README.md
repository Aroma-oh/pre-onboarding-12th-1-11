...
* 주요 변경 사항은, Container에서 관리하던 여러 상태들을 useAuth 훅으로 분리하여 다시한번 로직 분리 및 코드의 가독성을 높인 점입니다.
* 아쉬운 점은 error message 상태는 여전히 Container가 관리하며, error message 컴포넌트가 AuthInput 하위 컴포넌트로 종속되어 있다는 점입니다.
#### 협업 과정 돌아보기
